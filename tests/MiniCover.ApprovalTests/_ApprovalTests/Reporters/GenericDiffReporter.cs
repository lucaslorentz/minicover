            ...
            Func<IEnumerable<string>> allowedFileTypes)
        {
            if (diffProgram == null)
            {
                throw new NullReferenceException(
                    $@"Illegal arguments for {GetType().Name} (diffProgam, argumentsFormat, diffProgramNotFoundMessage)
Recieved {GetType().Name} ({diffProgram}, {argumentsFormat}, {diffProgramNotFoundMessage})");
            }
        ...
        private static void Launch(LaunchArgs launchArgs, bool waitForExit)
        {
            ...
            {
                throw new Exception(
                    $"Unable to launch: {launchArgs.Program} with arguments {launchArgs.Arguments}\nError Message: {e.Message}",
                    e);
            }
        }
    }
